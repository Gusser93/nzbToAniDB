import os.path, time, re, urllib.request, urllib.error, urllib.parse
	...
	def find_tvdb(self,anidbid,anidbepnum):
		...
		for ep in eps:
			...
			if tvdbseason == 'a' and anidbseason == '1':
				...
				episodedata = ET.fromstring(urllib.request.urlopen(absoluteurl).read())
			...
			try:
				...
				try:
					seriesdata = ET.fromstring(urllib.request.urlopen(seriesurl).read())
			...
			try:
				...
				try:
					episodedata = ET.fromstring(urllib.request.urlopen(episodeurl).read())
				except urllib.error.HTTPError as e:
					return None
